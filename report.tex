...
\subsection{Classification}
   ...
   2) Construct multiple logistic regressions using OneVsRest Classifier for each genre to predict the label. 
   
   3) Evaluating the model use f-1 score. We also use some costumed indicator - percentage of correctly predicted label and 
   
   We further develop our model trying different algorithms in OneVsRest Classifier including LinearSVC and neural network classifier. Also we perform GridSearch for hyper-parameters tuning, trying out different regularization penalty for best performances.
   We construct the text feature using all non-stopwords in plot summaries as well as phrases of these summaries returned by Autophrase. We then compare the results using these two features to explore the contribution of phrases in the model.
